...
export function truncate(text: string, max = MATERIAL_LINE_CHARS): string {
...
function userText(event: SessionEvent<'user/message'>): string {
  return extractText((event.data as UserMessage).content)
}
 ...
 */
export function materialFromEvents(events: readonly SessionEvent[]): MaterialLine[] {
...
export function materialStats(lines: readonly MaterialLine[]): MaterialStats {
  ...
  for (const line of lines) {
    switch (line.kind) {
      case 'user': userMessageCount += 1; break
      case 'assistant': assistantMessageCount += 1; break
      case 'tool': toolCallCount += 1; break
 ...
 */
export function selectWindow(events: readonly SessionEvent[], sinceSeq?: number): SessionEvent[] {
  const tail = events.slice(-MATERIAL_WINDOW)
  if (sinceSeq === undefined) return tail
