...
export const STYLE_MATRIX: readonly DreamStyleDef[] = [
  {
    id: 'cyberpunk',
    nameZh: '赛博朋克 / 废土',
 ...
 */
export function heuristicMoodHints(stats: MaterialStats): readonly string[] {
  const hints: string[] = []
...
export function fallbackStyle(stats: MaterialStats): DreamStyle {
  if (stats.errorCount >= 3) return 'cyberpunk'
  if (stats.errorCount > 0) return 'noir'
  if (stats.eventCount === 0) return 'surreal'
  return 'fantasy'
}
