...
import { generateDream } from './engine/dreamer.ts'
import { DreamStore } from './store.ts'
import { registerDreamWebUi } from './webui/server.ts'
import type {
...
export const DreamIncubatorConfigFields = {
  ...
  noiseIntensity: z.union(['low', 'medium', 'high'] as const).required(),
 ...
 */
export function resolveDreamIncubatorConfig(config: DreamIncubatorConfig): DreamIncubatorConfig {
  ...
  for (const key of Object.keys(value)) {
    if (!CONFIG_KEYS.has(key)) throw new Error(`dream-incubator: unknown config key "${key}"`)
  }
...
/** Per-session cadence bookkeeping for the incubation-effect policy. */
interface CadenceState {
  /** Last attempt (success or failure), ms since epoch; gates the cooldown. */
  lastAttemptAt: number
  /** A cycle is in flight for this session. */
  running: boolean
}
 ...
 */
export function apply(ctx: Context, config: DreamIncubatorConfig): void {
  ...
  ctx.effect(() => {
    ...
    const cadence = new Map<string, CadenceState>()
    const lifecycle = new AbortController()

    /** The WebUI push channel; a no-op when the UI is disabled. */
    let push: (record: DreamRecord) => void = () => {}
    if (resolved.serveUi) {
     ...
     */
    function gateReason(session: Session, state: CadenceState | undefined): GateReason | undefined {
      if (state !== undefined) {
    ...
    async function dreamFor(
      ...
      state.running = true
      try {
        const record = await generateDream(
          ctx,
          resolved,
          session,
          store.lastMaterialSeq(session.id),
          Math.random,
          externalSignal ?? lifecycle.signal,
        )
        ...
        logger.warn(`dream cycle failed for ${session.id}: ${(error as Error).message}`)
        return undefined
      } finally {
        state.running = false
      }
