 ...
 */
const WEBUI_CANDIDATES = ['./webui/', '../webui/', '../../webui/'] as const
...
function serveAsset(res: ServerResponse, relativePath: string): void {
...
function joinWithin(root: string, relativePath: string): string {
 ...
 */
export function registerDreamWebUi(
  ctx: Context,
  store: DreamStore,
  ...
  const push = (record: DreamRecord): void => {
    ...
    handler: async (req, res) => {
      ...
      if (req.method === 'POST') {
        try {
          ...
          const dreamId: DreamId = brandDreamId(id)
