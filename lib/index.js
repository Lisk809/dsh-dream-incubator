...
*/
function mergedStyleMatrix(userStyles) {
	if (userStyles === void 0 || userStyles.length === 0) return STYLE_MATRIX;
	const merged = STYLE_MATRIX.slice();
	for (const custom of userStyles) merged.push({
		...custom,
...
function isDreamStyle(value, matrix) {
...
*/
function rotationOffset(rotationDays, epochDays, matrix) {
	return Math.floor(epochDays / Math.max(1, Math.floor(rotationDays))) % matrix.length;
}
/**
...
*/
function rotatedStyles(offset, matrix) {
...
*/
function coerceScan(raw, matrix) {
	...
	const style = isDreamStyle(record["style"], matrix) ? record["style"] : void 0;
...
async function runScan(ctx, config, route, sessionId, request, matrix, signal) {
	return streamText(ctx, frameCall(route, sessionId, "You are a dream analyst. Follow the contract exactly.", scanPrompt(request, rotatedStyles(request.rotationOffset, matrix), heuristicMoodHints(request.stats)), config.maxOutputTokens, signal), signal);
}
/** Run the generation stage. */
async function runDream(ctx, config, route, sessionId, request, scan, matrix, signal) {
	return streamText(ctx, frameCall(route, sessionId, "You are the dreaming part of an AI agent. Follow the contract exactly.", dreamPrompt(request, styleDef(scan.style, matrix), scan.moodLabel, scan.themes), config.maxOutputTokens, signal), signal);
}
...
* @param matrix - the effective style library the scan style is checked against.
*/
function parseScan(rawText, matrix) {
	let parsed;
	try {
...
async function generateDream(ctx, config, session, sinceSeq, rng = Math.random, externalSignal) {
	...
	try {
		try {
			...
			const scan = parseScan(await runScan(ctx, config, route, session.id, request, matrix, callDeadline.signal), matrix);
			const { title, text } = splitDreamOutput(await runDream(ctx, config, route, session.id, request, scan, matrix, callDeadline.signal));
			return {
	...
	disposers.push(httpServer.register({
		...
		handler: (req, res) => {
			...
			writeJson(res, 200, { settings: {
				...
				styles: mergedStyleMatrix(config.styles).map(({ id, nameZh, palette }) => ({
					id,
					nameZh,
					palette
				}))
...
function resolveUserStyles(stylesRaw) {
	if (stylesRaw === void 0) return void 0;
	if (!Array.isArray(stylesRaw)) throw new Error("dream-incubator: styles must be an array of style definitions");
	const seen = new Set(DREAM_STYLES);
	return stylesRaw.map((entry, index) => {
		const where = `styles[${index}]`;
		if (entry === null || typeof entry !== "object") throw new Error(`dream-incubator: ${where} must be an object`);
		const def = entry;
		const id = def["id"];
		if (!isNonEmptyString(id)) throw new Error(`dream-incubator: ${where}.id must be a non-empty string`);
		if (seen.has(id)) throw new Error(`dream-incubator: duplicate dream style "${id}"`);
		seen.add(id);
		const nameZh = def["nameZh"];
		if (!isNonEmptyString(nameZh)) throw new Error(`dream-incubator: ${where}.nameZh must be a non-empty string`);
		const nameEn = def["nameEn"];
		if (!isNonEmptyString(nameEn)) throw new Error(`dream-incubator: ${where}.nameEn must be a non-empty string`);
		const trigger = def["trigger"];
		if (typeof trigger !== "string" || !DREAM_STYLE_TRIGGERS.includes(trigger)) throw new Error(`dream-incubator: ${where}.trigger must be one of "fatigue", "joy", "anxiety", "boredom", "confusion", "conflict"`);
		const imagery = def["imagery"];
		if (!Array.isArray(imagery) || imagery.length === 0 || !imagery.every(isNonEmptyString)) throw new Error(`dream-incubator: ${where}.imagery must be a non-empty array of non-empty strings`);
		const palette = def["palette"];
		if (palette !== void 0 && !isNonEmptyString(palette)) throw new Error(`dream-incubator: ${where}.palette must be a non-empty string when supplied`);
		return {
			id,
			nameZh,
			nameEn,
